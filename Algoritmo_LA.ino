...
float LMax_Dinamico = 0;
float LMin_Dinamico = 0;
...
void loop()
{
  ...
  if(flag == 1)
    ...
    // MÉDIA ADAPTATIVA ------------------------------------------------------
    EEG_Media = ((1.0 - alpha) * EEG_Media) + (alpha * EEG);

    // DESVIO ADAPTATIVO -----------------------------------------------------
    EEG_Desvio = ((1.0 - alpha) * EEG_Desvio) + (alpha * abs(EEG - EEG_Media));

    // LIMIARES DINÂMICOS ----------------------------------------------------
    LMax_Dinamico = EEG_Media + (ganho * EEG_Desvio);

    LMin_Dinamico = EEG_Media - (ganho * EEG_Desvio);
    ...
    // DETECÇÃO --------------------------------------------------------------
    Peak_Finder(EEG, EEG_Old, EEG_Diff, EEG_Diff_Old, LMax_Dinamico, LMin_Dinamico);

    // SERIAL PLOTTER --------------------------------------------------------
    Serial.print(EEG);
    Serial.print(",");

    Serial.print(LMax_Dinamico);
    Serial.print(",");

    Serial.print(LMin_Dinamico);
    Serial.print(",");

    Serial.print(EEG_Diff + EEG_Media);
    Serial.print(",");

    Serial.println((flagUltra * 1000) + EEG_Media);
